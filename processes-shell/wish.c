...
cmd_t *create_cmd(void) {
  ...
  if (cmd->arg == NULL)
    err_msg(1, true);
  for (size_t i = 0; i < MAX_ARGS_NUM; i++)
    cmd->arg[i] = NULL;

  cmd->fname = (char **)malloc(MAX_REDIR_FILES * sizeof(char *));
  if (cmd->fname == NULL)
    err_msg(1, true);
  for (size_t i = 0; i < MAX_REDIR_FILES; i++)
    cmd->fname[i] = NULL;
...
void free_parallel_cmds(char **pcmds) { free(pcmds); }
...
int cmd_avail(char **paths, cmd_t *cmd) {
  ...
  for (size_t i = 0; paths[i] != NULL; i++) {
    ...
    if (cmd->arg[0] != NULL) {
      ...
      if (access((const char *)cmd->abs_name, X_OK) == 0)
        return 0;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if ((paths = create_search_paths()) == NULL)
    err_msg(1, true);
  if (initialize_search_path(paths) != 0)
    err_msg(1, true);
  while (eof != -1) {
